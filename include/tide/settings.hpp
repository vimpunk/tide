...
struct disk_io_settings
{
    ...
    // pulled in a time. This will drastically affect download performance, but may be
    // necessary in memory constrained systems.
    int read_cache_line_size = values::none;

    // The number of contiguous 16KiB blocks that are buffered before written to disk.
    // If the piece is smaller than this, the number of blocks in piece is used instead
    // of this value.
    ...
    // `peer_session`'s receive buffer, which means that the write cache would never
    // fill up as `peer_session` would not be able to receive more blocks, effectively
    // stalling the download. For this reason if this condition is not met, it will be
    // overwritten by a value deemed fit by the implementation.
    //
...
struct torrent_settings
{
    ...
    // fully downloaded. Take everything, give nothing back, eh?
    bool stop_when_downloaded = false;

    // The following fields are set and managed by the implementation if values are
    // `values::none`.

    // The number of peers to which we upload pieces. This should not be a high value
    // for better performance, unless a lot of uplink bandwidth is available.
...
struct settings
{
    ...
    // The port number to which the tide's listener will attempt to bind. If met with
    // failure or no port is specified, it falls back to the OS provided random port,
    // which is then assigned to this field.
    //
    // NOTE: currently this field may only be set the first time, as changing it once
    // we're bound to it leads to some complications (e.g. letting know all trackers).
    ...
    // The maximum upload and download speeds of all torrents combined. These may be
    // overwritten by individual torrents. See comment above `torrent` below.
    int max_download_rate = values::unlimited;
    int max_upload_rate = values::unlimited;

    // A torrent is stopped (but not removed) if it reaches either of the following values:
    //
    ...
    // Global default settings for all torrents, but each individual torrent's settings
    // may be customized.
    // If `max_download_rate` and/or `max_upload_rate` are other than `values::none`,
    // the same fields in `settings` are disregarded, as each torrent will then employ
    // its own upload/download rate limits.
    torrent_settings torrent;
    // Global settings for all `peer_session` instances. It is not possible to customize
    // a torrent's peer_sessions' settings, all of them refer to this instance.
