...
// Common constructor.
torrent::torrent(torrent_id_t id, const int num_pieces, asio::io_context& ios,
        disk_io& disk_io, rate_limiter& global_rate_limiter,
        const settings& global_settings, engine_info& global_info,
...
// For new torrents.
torrent::torrent(torrent_id_t id, asio::io_context& ios, disk_io& disk_io,
        rate_limiter& global_rate_limiter, const settings& global_settings,
        engine_info& global_info, std::vector<tracker_entry> trackers,
        endpoint_filter& endpoint_filter, alert_queue& alert_queue, torrent_args args)
    : torrent(id, args.metainfo.num_pieces, ios, disk_io, global_rate_limiter,
              global_settings, global_info, std::move(trackers), endpoint_filter,
              alert_queue)
{
...
void torrent::apply_torrent_args(torrent_args& args)
{
    ...
    info_.info_hash = create_sha1_digest(args.metainfo.source.find_bmap("info").encode());
    ...
    info_.last_piece_length = info_.size - (info_.num_pieces - 1) * info_.piece_length;
    info_.num_blocks
            = (info_.num_pieces) * util::ceil_division(info_.piece_length, 0x4000)
...
void torrent::start()
{
    if(info_.state[torrent_info::active]) {
        return;
    }
...
void torrent::force_tracker_announce(string_view url)
{
    ...
    if(entry != trackers_.end()) {
        log(log_event::tracker, log::priority::high, "sending event(none) to tracker(%s)",
                url.data());
        entry->tracker->announce(prepare_tracker_request(tracker_request::none),
                [SHARED_THIS, entry](const error_code& ec, tracker_response r) {
...
void torrent::make_file_top_priority(const int file_index)
{
    ...
    if(!pieces.empty()) {
        piece_picker_.make_top_priority(pieces);
    }
}
...
void torrent::prioritize_file(const int file_index)
{
    const interval pieces = storage_.pieces_in_file(file_index);
    if(!pieces.empty()) {
        piece_picker_.make_priority(pieces);
    }
}
...
void torrent::deprioritize_file(const int file_index)
{
    const interval pieces = storage_.pieces_in_file(file_index);
    if(!pieces.empty()) {
        piece_picker_.clear_priority(pieces);
    }
}
...
void torrent::apply_settings(const torrent_settings& s)
{
    ...
    if(is_seed() && s.stop_when_downloaded) {
        stop();
    }
...
inline void torrent::on_torrent_allocated(
        const error_code& error, torrent_storage_handle storage)
{
    if(error || !storage) {
        ...
        if(info_.num_disk_io_failures < 100) {
            ...
            //[SHARED_THIS](const error_code& ec, torrent_storage_handle storage)
    ...
    for(const file_index_t file : info_.priority_files) {
        ...
        log(log_event::disk, "making file (%s) priority", info_.files[file].path.c_str());
...
inline void torrent::save_resume_data()
{
    ...
    disk_io_.save_torrent_resume_data(id(), create_resume_data(),
            [SHARED_THIS](const error_code& error) { on_resume_data_saved(error); });
    has_state_changed_ = false;
    info_.state[torrent_info::saving_state] = true;
    info_.last_resume_data_save_time = cached_clock::now();
}

void torrent::on_resume_data_saved(const error_code& error)
{
...
bmap_encoder torrent::create_resume_data() const
{
    ...
    resume_data["info_hash"]
            = std::string(info_.info_hash.begin(), info_.info_hash.end());
    resume_data["bitfield"]
            = is_seed() ? "have_all" : piece_picker_.my_bitfield().to_string();
    ...
    resume_data["download_started_time"] = to_int<seconds>(info_.download_started_time);
    resume_data["download_finished_time"] = to_int<seconds>(info_.download_finished_time);
...
void torrent::restore_resume_data(const bmap& resume_data)
{
    ...
    */
    ...
    // settings
    resume_data.try_find_number(
            "download_sequentially", info_.settings.download_sequentially);
    resume_data.try_find_number(
            "stop_when_downloaded", info_.settings.stop_when_downloaded);
    ...
    resume_data.try_find_number(
            "total_downloaded_piece_bytes", info_.total_downloaded_piece_bytes);
    resume_data.try_find_number(
            "total_uploaded_piece_bytes", info_.total_uploaded_piece_bytes);
    resume_data.try_find_number("total_downloaded_bytes", info_.total_downloaded_bytes);
    resume_data.try_find_number("total_uploaded_bytes", info_.total_uploaded_bytes);
    resume_data.try_find_number(
            "total_verified_piece_bytes", info_.total_verified_piece_bytes);
    resume_data.try_find_number(
            "total_failed_piece_bytes", info_.total_failed_piece_bytes);
    resume_data.try_find_number("total_wasted_bytes", info_.total_wasted_bytes);
    resume_data.try_find_number(
            "total_bytes_written_to_disk", info_.total_bytes_written_to_disk);
    resume_data.try_find_number(
            "total_bytes_read_from_disk", info_.total_bytes_read_from_disk);
...
void torrent::announce(const int event, const bool force)
{
    ...
    // If the event is stopped or completed, we need to send it to all trackers
    // to which we have announced in the past, otherwise just pick the most
    // suitable tracker, as we're just requesting more peers.
    if((event == tracker_request::stopped) || (event == tracker_request::completed)) {
        for(auto& entry : trackers_) {
            ...
            if((entry.has_sent_completed && event == tracker_request::completed)
                ...
                entry.tracker->announce(request,
                        [SHARED_THIS, &entry, event](
                                const error_code& ec, tracker_response r) {
        ...
        if(!t) {
            if(trackers_.empty() && peer_sessions_.empty() && available_peers_.empty()) {
                log(log_event::tracker, log::priority::high,
                        "couldn't find tracker to announce to and we don't have"
                        " any peers, stopping torrent");
                stop();
            }
        ...
        log(log_event::tracker, log::priority::high, "sending event(%s) to tracker(%s)",
...
inline tracker_request torrent::prepare_tracker_request(const int event) const noexcept
{
    ...
    request.num_want = event == tracker_request::started || event == tracker_request::none
            ? calculate_num_want()
            : 0;
    return request;
}
...
inline int torrent::calculate_num_want() const noexcept
{
    ...
    const int num_desired = info_.settings.max_connections - peer_sessions_.size()
...
void torrent::on_announce_response(tracker_entry& tracker, const error_code& error,
        tracker_response response, const int event)
{
    ...
    available_peers_.reserve(available_peers_.size() + response.ipv4_peers.size()
            + response.ipv6_peers.size());
...
inline void torrent::on_announce_error(
    ...
    if(endpoint_filter_.is_allowed(peer)) {
        ...
        auto psit = std::find_if(peer_sessions_.begin(), peer_sessions_.end(),
                [&peer](const auto& session) {
                    return session->remote_endpoint() == peer;
                });
...
void torrent::update(const error_code& error)
{
    if(error) {
    ...
    // If we have no peers, announce to tracker, and if it doesn't give us
    // peers, in the next update announce to another tracker. repeat this until
    // we get peers or we've exhausted all our trackers, but don't force
    // announce, because that'd override this.
    if(should_connect_peers()) {
        connect_peers();
    }
    if(needs_peers()) {
        announce(tracker_request::none);
        log(log_event::update, "%i peers (max: %i), announcing", total_peers(),
                info_.settings.max_connections == values::none
                        ? info_.settings.max_connections
                        : global_settings_.max_connections);
    }

    // Only run the unchoke procedure every 10 seconds.
    if(cached_clock::now() - info_.last_unchoke_time >= seconds(10)) {
        unchoke();
    }
    ...
    if(is_seed()) {
        ...
        if(has_reached_share_ratio_limit() || has_reached_seed_time_limit()
    ...
    start_timer(update_timer_, seconds(1),
            [SHARED_THIS](const error_code& error) { update(error); });
}
...
void torrent::connect_peers()
{
    ...
    available_peers_.erase(
            available_peers_.begin(), available_peers_.begin() + num_to_connect);
}
...
inline void torrent::connect_peer(tcp::endpoint& peer)
{
    ...
    peer_sessions_.emplace_back(std::make_shared<peer_session>(ios_, std::move(peer),
            local_rate_limiter_, global_settings_.peer_session, torrent_frontend(*this)));
...
inline void torrent::sort_unchoke_candidates(const int n)
{
    ...
    std::partial_sort(peer_sessions_.begin(), peer_sessions_.begin() + n,
            peer_sessions_.end(),
            [this](const auto& a, const auto& b) { return unchoke_comparator_(*a, *b); });
}
...
void torrent::unchoke()
{
    ...
    if((num_to_unchoke == 0) && (info_.num_unchoked_peers == 0)) {
        return;
    }
...
{
    if(peer_sessions_.size() == info_.num_unchoked_peers) {
        return;
    }
    ...
    const auto for_each_candidate = [this](const auto& f) {
        for(auto& session : peer_sessions_) {
            if(session->is_connected() && !session->is_peer_seed()
                    && session->is_peer_choked() && session->is_peer_interested()) {
                f(session.get());
            }
        }
    };
    ...
    if(!candidates.empty()) {
        std::sort(candidates.begin(), candidates.end(), [](const auto& a, const auto& b) {
            return a->connection_established_time() < b->connection_established_time();
        });
        // Pick peer by generating a random number skewed towards the lower end,
        // thereby giving newer peers a better chance at being picked.
        const int peer_index
                = std::floor(candidates.size() * std::pow(util::random_real(), 2));
...
{
    // Instead of using weighed upload rate averages for determining a peer's
    // bandwidth between unchoke rounds, we instead count the number of bytes
    // that were sent during this period, and disregard past performance.  Note
    // to self: function is called upload based but we query downloaded bytes;
    // function name refers to peer's upload capacity, while the query functions
    // are from our perspective TODO perhaps change this to be less confusing.
...
void torrent::on_new_piece(piece_download& download, const bool is_valid)
{
    ...
    const auto it = std::find_if(
            downloads_.begin(), downloads_.end(), [&download](const auto& d) {
                return d->piece_index() == download.piece_index();
            });
    // If we could not find this piece download it means that it was
    // a parole/unique download, i.e. `peer_session` didn't put it in downloads_
    // because it wanted to test whether peer was the one sending corrupt data,
    // and for this the piece download was not shared among other
    // `peer_session`s.
    ...
    for(const auto& d : downloads_) {
        ...
        s += std::to_string(d->piece_index()) + "-(" + std::to_string(num_have_blocks)
                ...
                + std::to_string(d->peers().size()) + ") ";
    }
    if(!s.empty())
        log(log_event::download, log::priority::high, "active piece downloads: %s",
                s.c_str());
...
inline void torrent::handle_valid_piece(piece_download& download)
{
    ...
    for(file_index_t i = files.begin; i < files.end; ++i) {
        const auto slice = storage_.get_file_slice(
                i, block_info(download.piece_index(), 0, piece_length));
    ...
    } else if(!info_.settings.download_sequentially && piece_picker_.num_have_pieces() > 4
...
template <typename... Args>
void torrent::log(const log_event event, const log::priority priority, const char* format,
        Args&&... args) const
{
#ifdef TIDE_ENABLE_LOGGING
