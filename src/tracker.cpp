    ...
    std::function<void(const std::error_code&, scrape_response)> handler)
{
    ...
    {
        execute_request(
            create_request_entry<scrape_request>(
    ...
    request& request, const size_t num_bytes_received)
{
    ...
    log(log_event::incoming,
        "received CONNECT (trans_id: %i, conn.id: %i)",
...
void udp_tracker::send_announce_request(announce_request& request)
{
    ...
    log(log_event::outgoing,
        ...
        params.event == tracker_request::event_t::started
            ? "started"
            : params.event == tracker_request::event_t::completed
                ? "completed"
                : params.event == tracker_request::event_t::stopped
                    ? "stopped"
                    : "none",
    ...
    announce_request& request, const size_t num_bytes_received)
{
    ...
    log(log_event::incoming,
        "received ANNOUNCE (trans_id: %i; interval: %i; num_leechers: %i; num_seeders: %i;"
        " num_peers: %i",
...
inline void udp_tracker::receive_message()
{
    ...
    m_socket.async_receive(
        asio::buffer(*m_receive_buffer, m_receive_buffer->max_size()),
    ...
    const std::error_code& error, const size_t num_bytes_received)
{
    ...
    log(log_event::incoming,
        "received %i bytes (trans_id: %i; action: %i)",
    ...
    case action_t::announce_1:
        handle_announce_response(
            static_cast<announce_request&>(request),
            num_bytes_received
        );
        receive_message();
        break;
    ...
    request& request, const size_t num_bytes_received)
{
    ...
    if(dynamic_cast<announce_request*>(&request))
    {
        ...
        log(log_event::incoming,
            "received announce ERROR (trans_id: %i; error_msg: %s)",
    ...
    else
    {
        ...
        r.failure_reason = std::string(buffer, buffer + error_msg_len);
        log(log_event::incoming,
            "received scrape ERROR (trans_id: %i; error_msg: %s)",
            request.transaction_id, r.failure_reason.c_str()
        );
