...
peer_session::peer_session(asio::io_context& ios, tcp::endpoint peer_endpoint,
    ...
    : peer_session(ios, std::move(peer_endpoint), rate_limiter, settings)
{
    ...
    info_.available_pieces = bitfield(torrent_.piece_picker().num_pieces());
...
peer_session::~peer_session()
{
    ...
    for(auto& download : downloads_) {
        download->deregister_peer(remote_endpoint());
    }
...
void peer_session::start()
{
    if(is_disconnected()) {
        ...
        // receive cycle.
        if(!op_state_[op::receive] && !op_state_[op::disk_write]) {
            receive();
        }
...
inline void peer_session::connect()
{
    ...
    socket_->async_connect(info_.remote_endpoint,
            [SHARED_THIS](const error_code& error) { on_connected(error); });

    info_.state = state::connecting;
    info_.connection_started_time = cached_clock::now();
    if(torrent_) {
        ++torrent_.info().num_connecting_sessions;
    }

    start_timer(timeout_timer_, settings_.peer_connect_timeout,
            [SHARED_THIS](const error_code& error) { on_connect_timeout(error); });
    log(log_event::connecting, log::priority::low, "started establishing connection");
}

void peer_session::on_connected(const error_code& error)
{
    if(torrent_) {
        --torrent_.info().num_connecting_sessions;
    }
    ...
    log(log_event::connecting, "connected in %lims",
            to_int<milliseconds>(
                    info_.connection_established_time - info_.connection_started_time));
    ...
    start_timer(timeout_timer_, settings_.peer_timeout,
            [SHARED_THIS](const error_code& error) { on_inactivity_timeout(error); });
}

inline bool peer_session::should_abort(const error_code& error) const noexcept
{
    return (error == asio::error::operation_aborted) || is_stopped();
}

void peer_session::disconnect(const error_code& error)
{
    if(is_disconnected()) {
        return;
    }
    ...
    log(log_event::disconnecting, log::priority::high, "reason: %s (#%i)", reason.c_str(),
            error.value());
#endif // TIDE_ENABLE_LOGGING
    ...
    if(parole_download_) {
        detach_parole_download();
    }
    if(torrent_) {
        torrent_.piece_picker().decrease_frequency(info_.available_pieces);
        // It's possible that `is_peer_seed_` is not set at this point, i.e. we
        // disconnected before establishing whether peer is a seed or leech, in which
        // case we must not decrement any of these values.
        if(info_.state != state::connected) {
            if(is_peer_seed())
                --torrent_.info().num_seeders;
            else
                --torrent_.info().num_leechers;
        }
        ...
        if(info_.in_transit_block != invalid_block) {
            ...
            torrent_.info().total_wasted_bytes += message_parser_.size();
        }
...
{
    s.torrent_id = torrent_ ? torrent_.id() : -1;
...
{
    if(is_connected() && is_extension_enabled(extensions::fast)
    ...
    if(is_connected()) {
        ...
        if(torrent_.piece_picker().has_all_pieces() && !downloads_.empty()) {
            ...
            log(log_event::info, log::priority::high,
                    "became seeder but have %i downloads left: %s", downloads_.size(),
                    s.c_str());
        }
...
void peer_session::update_interest()
{
    ...
    const bool am_interested
            = torrent_.piece_picker().am_interested_in(info_.available_pieces);
    if(!was_interested && am_interested) {
        ...
        if(!is_peer_interested()) {
            start_timer(timeout_timer_, inactivity_timeout_value(),
                    [SHARED_THIS](
                            const error_code& error) { on_inactivity_timeout(error); });
        }
        log(log_event::info, "no longer interested in peer (have: %i, has %i pieces)",
                torrent_.piece_picker().num_have_pieces(),
...
void peer_session::send()
{
    ...
    socket_->async_write_some(send_buffer_.get_buffers(num_bytes_to_send),
            [SHARED_THIS](const error_code& error, size_t num_bytes_sent) {
                on_sent(error, num_bytes_sent);
            });

    op_state_.set(op::send);

    log(log_event::outgoing, log::priority::low, "sending: %i; available: %i; quota: %i",
...
void peer_session::on_sent(const error_code& error, size_t num_bytes_sent)
{
    ...
    log(log_event::outgoing,
            "sent: %i; quota: %i; send buffer size: %i; total sent: %lli", num_bytes_sent,
            info_.send_quota, send_buffer_.size(), info_.total_uploaded_bytes);

    // This call to `send` will only write to socket again if during the first
    // write there were more bytes in send buffer to send than we had quota for,
    // and since the first thing in send is asking for more bandwidth quota, we
    // may be able to send off the rest of the send buffer's contents.
    if(!send_buffer_.empty()) {
        send();
    }
}
...
void peer_session::receive()
{
    ...
    prepare_to_receive();
    const int num_to_receive
            = std::min(info_.receive_quota, message_parser_.free_space_size());
    if(num_to_receive == 0) // { return; }
    {
        // TODO remove after bug is found
        log(log_event::incoming, log::priority::high,
                "num_to_receive is 0! receive_quota: %i, "
                "message_parser_.free_space_size: %i",
    ...
    log(log_event::incoming, log::priority::low,
            "receiving: %i; receive buffer free space: %i; quota: %i", num_to_receive,
            message_parser_.free_space_size(), info_.receive_quota);

    // Guard against inactivity/slow response.
    start_timer(timeout_timer_, settings_.peer_timeout,
            [SHARED_THIS](const error_code& error) { on_inactivity_timeout(error); });
}
...
void peer_session::prepare_to_receive()
{
    ...
    const int num_to_receive = std::min(
            num_expected_bytes_to_receive(), effective_receive_buffer_capacity());
    ...
    if(quota_needed > 0) {
        request_receive_quota(quota_needed);
    }
...
inline int peer_session::num_expected_bytes_to_receive() const noexcept
{
    ...
    // Some fallback value, may need to change.
    return 256;
}
...
inline int peer_session::effective_receive_buffer_capacity() const
{
    ...
    int buffer_capacity = settings_.max_receive_buffer_size - message_parser_.size()
...
inline void peer_session::request_receive_quota(const int num_bytes)
{
    ...
    log(log_event::incoming, log::priority::high,
            "receive quota:: requested: %i, received: %i, total: %i", num_bytes, quota,
            info_.receive_quota);
...
{
    // We are gracefully stopping and if there are no other pending async ops,
    // we can shut down.
...
void peer_session::on_received(const error_code& error, size_t num_bytes_received)
{
    error_code ec;
    timeout_timer_.cancel(ec);
    ...
    // TODO maybe turn this into a loop
    if(message_parser_.is_full()) {
        // Handle whatever messages we already have before flushing the rest of
        // socket's buffer, to avoid reallocating our receive buffer.
        handle_messages();
...
inline int peer_session::flush_socket()
{
    ...
    if(ec) {
        ...
        if((ec == asio::error::would_block) || (ec == asio::error::try_again)) {
            ...
            log(log_event::incoming, log::priority::low, "sync read %i bytes from socket",
                    num_bytes_read);
        ...
        const bool was_choked, const size_t num_bytes_received)
{
    ...
    if(old_buffer_size != message_parser_.buffer_size())
        log(log_event::incoming, log::priority::low, "grew receive buffer from %i to %i",
    ...
    if(!was_choked && am_choked() && old_buffer_size > 1024) {
        // If we went from unchoked to choked (and if buffer is large enough,
        // otherwise don't bother), ~100 bytes should suffice to receive further
        // protocol chatter (if we have unfinished messages in receive buffer it
        // will not shrink below the last valid message byte).
        message_parser_.shrink_to_fit(128);
        log(log_event::incoming, log::priority::low,
                "shrunk receive buffer from %i to %i", old_buffer_size,
                message_parser_.buffer_size());
    }
}
...
inline void peer_session::handle_messages()
{
    if(info_.state == state::handshaking) {
        ...
        if(is_disconnected() || (info_.state == state::handshaking)) {
            return;
        }
    }
    if(info_.state == state::piece_availability_exchange
            && message_parser_.has_message()) {
        piece_availability_exchange_stage();
        if(is_disconnected()) {
            return;
        }
    }
...
#undef NOT_AFTER_HANDSHAKE
    ...
    if(info_.num_outstanding_bytes > 0) {
        probe_in_transit_block();
    }
}

inline void peer_session::handshake_stage()
{
...
inline void peer_session::piece_availability_exchange_stage()
{
    // TODO are we sending our piece availability?
    const auto msg_type = message_parser_.type();
    ...
    info_.state = state::connected;
    // We can now establish if peer is a seed or not. If peer is a leech, we need
    // to update this field once peer becomes a seed.
    if(is_peer_seed())
        ++torrent_.info().num_seeders;
    else
        ++torrent_.info().num_leechers;
...
void peer_session::handle_handshake()
{
    ...
    std::copy(handshake.info_hash.cbegin(), handshake.info_hash.cend(),
            peer_info_hash.begin());
    if(info_.is_outbound) {
        // We started the connection, so we already sent our handshake,
        // so just verify peer's info_hash.
        if(peer_info_hash != torrent_.info_hash()) {
    ...
    start_timer(keep_alive_timer_, minutes(2),
            [SHARED_THIS](const error_code& error) { on_keep_alive_timeout(error); });
...
inline void peer_session::handle_bitfield()
{
    ...
    log(log_event::incoming, "BITFIELD (%s:%i)", is_peer_seed() ? "seed" : "leech",
            info_.available_pieces.count());
...
inline void peer_session::handle_request()
{
    ...
    if(!is_request_valid(block_info)) {
        log(log_event::invalid_message,
                "invalid REQUEST (piece: %i, offset: %i, length: %i)", block_info.index,
                block_info.offset, block_info.length);
        disconnect(peer_session_errc::invalid_request_message);
        return;
    }
    ...
    if(is_peer_choked()
            && (!is_extension_enabled(extensions::fast)
                       || std::find(outgoing_allowed_set_.begin(),
                                  outgoing_allowed_set_.end(), block_info.index)
                               == outgoing_allowed_set_.end())) {
        handle_illicit_request(block_info);
        return;
    }

    log(log_event::incoming, log::priority::high,
            "REQUEST (piece: %i, offset: %i, length: %i)", block_info.index,
            block_info.offset, block_info.length);
    ...
    if(should_accept_request(block_info)) {
        ...
        torrent_.fetch_block(
                block_info, [SHARED_THIS](const error_code& error, block_source block) {
...
inline void peer_session::handle_cancel()
{
    ...
    if(!is_request_valid(block_info)) {
        log(log_event::invalid_message,
                "invalid CANCEL (piece: %i, offset: %i, length: %i)", block_info.index,
                block_info.offset, block_info.length);
        disconnect(peer_session_errc::invalid_cancel_message);
        return;
    }
    ...
    auto request
            = std::find(incoming_requests_.begin(), incoming_requests_.end(), block_info);
    if(request != incoming_requests_.cend()) {
        ...
        if(is_extension_enabled(extensions::fast)) {
            send_reject_request(block_info);
        }
    }
}
...
void peer_session::handle_block()
{
    ...
    if(!is_block_info_valid(block_info)) {
        log(log_event::invalid_message,
                "invalid BLOCK (piece: %i, offset: %i, length: %i)", block_info.index,
                block_info.offset, block_info.length);
        disconnect(peer_session_errc::invalid_block_message);
        return;
    }
    log(log_event::incoming, log::priority::high,
            "BLOCK (piece: %i, offset: %i, length: %i)", block_info.index,
            block_info.offset, block_info.length);

    auto request
            = std::find(outgoing_requests_.begin(), outgoing_requests_.end(), block_info);
    ...
    // (`handle_reject_request` also erases it so no need to do this after
    // invoking it).
    outgoing_requests_.erase(request);

    // NOTE: we must update stats before adjusting request timeout and request
    // queue size as we adjust request timeout based on these stats.
    ...
    if(torrent_.piece_picker().my_bitfield()[block_info.index]) {
        ...
        // We MUST have download, because at this point block is deemed valid,
        // which means its request entry in `outgoing_requests_` was found,
        // meaning we expect this block, so its corresponding download instance
    ...
    if(can_make_requests())
        ...
        start_timer(request_timeout_timer_, request_timeout_value(),
                [SHARED_THIS](const error_code& error) { on_request_timeout(error); });
}
...
inline void peer_session::update_peer_timeout() // TODO rename
{
    ...
    const auto timeout = request_timeout_value();
...
inline void peer_session::adjust_best_request_queue_size() noexcept
{
    ...
    if(now - info_.last_request_queue_adjust_time < seconds(1)) {
        return;
    }
    ...
    if(op_state_[op::slow_start]) {
        // If our download rate is not increasing significantly anymore, exit
        // slow start.
        // FIXME due to the inaccuracy of updating the number of bytes
        // downloaded a second at varying intervals, the
        // per_second_downloaded_bytes counter is not accurate and sometimes
        // causes us to exit slow start early.
        if(deviation < 5000) {
            log(log_event::request, log::priority::high,
...
inline bool peer_session::is_block_info_valid(const block_info& block) const noexcept
{
    ...
    return is_piece_index_valid(block.index) && is_block_offset_valid
            && is_block_length_valid
            && block.length
            <= 0x8000; // TODO decide what the maximum block size should be
}

inline bool peer_session::is_piece_index_valid(const piece_index_t index) const noexcept
{
    return (index >= 0) && (index < torrent_.piece_picker().num_pieces());
}
...
inline void peer_session::handle_illicit_request(const block_info& block)
{
    ...
    if(cached_clock::now() - seconds(2) <= info_.last_outgoing_choke_time) {
        return;
    }
...
inline void peer_session::handle_have_all()
{
    ...
    assert(info_.available_pieces.size() == torrent_.piece_picker().num_pieces());
...
{
    log(log_event::incoming, log::priority::high,
            "REJECT REQUEST (piece: %i, offset: %i, length: %i)", request->index,
...
inline void peer_session::handle_allowed_fast()
{
    ...
    if(std::find(outgoing_allowed_set_.begin(), outgoing_allowed_set_.end(), piece)
            == outgoing_allowed_set_.end()) {
        ...
        const block_info& block_info, disk_buffer block_data, piece_download& download)
{
    ...
    // `parole_download_`, it is added to `shared_downloads_` if we disconnect
    ...
    torrent_.save_block(block_info, std::move(block_data), download,
            [SHARED_THIS, block_info, start_time = cached_clock::now()](
                    const error_code& ec) {
                on_block_saved(ec, block_info, start_time);
            });
        ...
        const bool is_piece_good, const int num_bytes_downloaded)
{
    ...
    const int n = std::count_if(outgoing_requests_.begin(), outgoing_requests_.end(),
            [&download](const auto& r) { return r.index == download.piece_index(); });
    if(n > 0) {
        log(log_event::info, log::priority::high,
                "%i blocks for piece(%i) in outgoing_requests_ after piece completion", n,
    ...
    if(it != downloads_.end()) {
        ...
        log(log_event::info, log::priority::high, "not removing download(%i)",
                download.piece_index());
#endif // TIDE_ENABLE_DEBUGGING
}
...
{
    log(log_event::disk, log::priority::high, "piece(%i) passed hash test",
            download.piece_index());

    info_.total_verified_piece_bytes
            += get_piece_length(torrent_.info(), download.piece_index());
...
{
    log(log_event::parole, log::priority::high, "piece(%i) failed hash test (%i fails)",
            download.piece_index(), info_.num_hash_fails);

    ++info_.num_hash_fails;
    info_.total_wasted_bytes += get_piece_length(torrent_.info(), download.piece_index());

    if(is_peer_on_parole() && parole_download_ && &download == parole_download_.get()) {
...
void peer_session::on_block_saved(
        const error_code& error, const block_info& block, const time_point start_time)
{
    info_.num_pending_disk_write_bytes -= block.length;
    // There may be multiple concurrent disk operations, so we can only unset
    // this state if we don't expect more bytes to be written to disk.
    if(info_.num_pending_disk_write_bytes == 0) {
        op_state_.unset(op::disk_write);
    }
    torrent_.info().num_pending_disk_write_bytes -= block.length;
    assert(error != disk_io_errc::invalid_block);
    // It's not really an error if piece turned out to be bad or it was
    // a duplicate block (though the latter should happen very rarely).
    if(error
            && (error != disk_io_errc::corrupt_data_dropped
                       && error != disk_io_errc::duplicate_block)) {
        // If block could not be saved we will have to redownload it at some
        // point, so tell its corresponding download instance to free this block
        // for requesting.
        if(error == disk_io_errc::block_dropped) {
            // The assert in find_download must not fire as in this case
            // (dropped block) we could not advance the hashing, i.e. the
            // completion of the piece, and a piece_download may only be removed
            // once we have fully hashed it.
        ...
        avg_disk_write_time_.update(
                to_int<milliseconds>(cached_clock::now() - start_time));

        log(log_event::disk, log::priority::high,
                "saved block to disk (piece: %i, offset: %i, length: %i) - "
                "disk write stats (total: %lli; pending: %lli)",
                block.index, block.offset, block.length,
                info_.total_bytes_written_to_disk, info_.num_pending_disk_write_bytes);
    }
...
void peer_session::on_block_fetched(const error_code& error, const block_source& block)
{
    if(error == disk_io_errc::operation_aborted) {
        ...
        ++info_.num_disk_io_failures;
        ++torrent_.info().num_disk_io_failures;
        log(log_event::disk, log::priority::high, "disk failure #%i",
                info_.num_disk_io_failures);
        if(info_.num_disk_io_failures > 100) {
            disconnect(error);
        }
    ...
    if(info_.num_pending_disk_read_bytes == 0) {
        op_state_.unset(op::disk_read);
    }
...
{
    send_buffer_.append(fixed_payload<9>().i32(5).i8(message::have).i32(piece));
    send();
...
void peer_session::send_request(const block_info& block)
{
    ...
    log(log_event::outgoing, "REQUEST (piece: %i, offset: %i, length: %i)", block.index,
...
inline void peer_session::update_upload_stats(const int num_bytes)
{
    ...
    log(log_event::outgoing, log::priority::high, "upload rate: %i bytes/s",
            info_.upload_rate.rate());
}
...
void peer_session::send_cancel(const block_info& block)
{
    ...
    if(block != info_.in_transit_block) {
        ...
        send();
        --torrent_.info().num_pending_blocks;
        auto it = std::find(outgoing_requests_.begin(), outgoing_requests_.end(), block);
        if(it != outgoing_requests_.end()) {
            outgoing_requests_.erase(it);
        }
...
{
    send_buffer_.append(fixed_payload<7>().i32(3).i8(message::port).i16(port));
    send();
...
{
    send_buffer_.append(fixed_payload<9>().i32(5).i8(message::suggest_piece).i32(piece));
    send();
...
{
    send_buffer_.append(fixed_payload<9>().i32(5).i8(message::allowed_fast).i32(piece));
    send();
...
void peer_session::send_allowed_fast_set()
{
    ...
    for(const auto& p : outgoing_allowed_set_) {
        send_allowed_fast(p);
    }
}
...
inline bool peer_session::can_make_requests() const noexcept
{
    ...
    return am_interested() && num_blocks_to_request() > 0
...
void peer_session::make_requests()
{
    ...
    for(auto& block : new_requests) {
        ...
        log(log_event::outgoing, log::priority::high,
                "REQUEST (piece: %i, offset: %i, length: %i)", block.index, block.offset,
                block.length);
        info_.num_outstanding_bytes += block.length;
        torrent_info.num_outstanding_bytes += block.length;
        ++torrent_info.num_pending_blocks;
    }
    log(log_event::outgoing, log::priority::high, "request queue length: %i",
            new_requests.size());
...
inline view<pending_block> peer_session::make_requests_in_parole_mode()
{
    ...
    if(parole_download_ == nullptr) {
        ...
        log(log_event::request, log::priority::high, "picked piece(%i) in parole mode",
                piece);
    }
    const int num_new_requests = parole_download_->pick_blocks(
            outgoing_requests_, remote_endpoint(), num_blocks_to_request());
    log(log_event::request, log::priority::high, "%i new parole requests",
            num_new_requests);
...
inline int peer_session::continue_downloads()
{
    ...
    for(auto& download : downloads_) {
        if(outgoing_requests_.size() >= info_.best_request_queue_size) {
            break;
        }
...
inline int peer_session::join_download()
{
    ...
    if(download) {
        log(log_event::request, log::priority::high, "joining piece(%i) download",
                download->piece_index());
        // It's safe to pass only this instead of SHARED_THIS because we remove
...
inline int peer_session::start_download()
{
    ...
    bool is_duplicate1
            = std::find_if(downloads.begin(), downloads.end(),
                      [piece](const auto& d) { return d->piece_index() == piece; })
            != downloads.end();
    bool is_duplicate2
            ...
            != downloads_.end();
    bool is_duplicate3
            = parole_download_ ? parole_download_->piece_index() == piece : false;
    ...
    if(piece != invalid_piece_index) {
        ...
        auto download
                = std::make_shared<piece_download>(piece, get_piece_length(info, piece));
        // It's safe to pass only this instead of SHARED_THIS because we remove
        // these callbacks from download when we destruct.
        download->register_peer(remote_endpoint(),
                [this, &download = *download](
...
inline int peer_session::num_blocks_to_request() const noexcept
{
...
inline duration peer_session::inactivity_timeout_value() const noexcept
{
    return 2 * settings_.peer_timeout;
}

void peer_session::on_request_timeout(const error_code& error)
{
    if(should_abort(error)) {
        return;
    } else if(error) {
    ...
    if(outgoing_requests_.empty())
        log(log_event::timeout, log::priority::high, "request queue is empty o.O");
    ...
    if(request != outgoing_requests_.end()) {
        ...
        if(download.time_out_request(*request)) {
            ...
            log(log_event::timeout, log::priority::high,
                    ...
                    download.num_blocks_left(), request->index, request->offset,
                    request->length,
                    to_int<seconds>(cached_clock::now() - request->request_time),
...
inline std::vector<pending_block>::iterator
peer_session::find_request_to_time_out() noexcept
{
    // TODO convert to rbegin/rend
...
inline seconds peer_session::request_timeout_value() const
{
...
void peer_session::on_connect_timeout(const error_code& error)
{
    if(should_abort(error)) {
        return;
    } else if(error) {
        // TODO verify this and below
        if(torrent_) {
            --torrent_.info().num_connecting_sessions;
        }
        disconnect(error);
        ...
        const auto elapsed
                = to_int<seconds>(cached_clock::now() - info_.connection_started_time);
        assert(elapsed > 0);
...
void peer_session::on_inactivity_timeout(const error_code& error)
{
    if(should_abort(error)) {
        return;
    } else if(error) {
...
void peer_session::on_keep_alive_timeout(const error_code& error)
{
    if(should_abort(error)) {
        return;
    } else if(error) {
    ...
    if(cached_clock::now() - info_.last_send_time > seconds(120)) {
        send_keep_alive();
    }
    start_timer(keep_alive_timer_, minutes(2),
            [SHARED_THIS](const error_code& error) { on_keep_alive_timeout(error); });
...
{
    const auto& torrent_info = torrent_.info();
    const auto& info_hash = torrent_.info_hash();
    const int n = std::min(torrent_info.num_pieces, settings_.allowed_fast_set_size);
    ...
    while(outgoing_allowed_set_.size() < n) {
        ...
        for(auto i = 0; (i < 5) && (outgoing_allowed_set_.size() < n); ++i) {
            ...
            if(outgoing_allowed_set_.end()
                    == std::find(outgoing_allowed_set_.begin(),
                               outgoing_allowed_set_.end(), index)) {
        ...
        const char* format, Args&&... args) const
{
    ...
    const torrent_id_t id = torrent_ ? torrent_.id() : -1;
...
inline piece_download& peer_session::find_download(const piece_index_t piece) noexcept
{
    auto it = std::find_if(downloads_.begin(), downloads_.end(),
            [piece](const auto& dl) { return dl->piece_index() == piece; });
