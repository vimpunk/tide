...
{
    return std::find_if(blocks.begin(), blocks.end(), [&block](const auto& b) {
        return &b.get() == &block;
    }) != blocks.end();
}
        ...
        std::function<void(const block_info&)> cancel_handler)
{
    ...
    assert(std::find_if(peers_.begin(), peers_.end(), [&id](const auto& p) {
        return p.id == id;
    }) == peers_.end());
...
void piece_download::got_block(const peer_id_type& id, const block_info& block_info)
{
    ...
    if(peer.num_bytes_downloaded == 0) {
        ++num_downloaders_;
    }
...
void piece_download::post_hash_result(const bool is_piece_good)
{
    ...
    if(num_blocks_left() != 0) {
        ...
        for(const auto b : blocks_) {
            std::cout << "b" << i << "("
                      << (b.status == block::status::free
                                         ? "free"
                                         : b.status == block::status::requested
                                                 ? "requested"
                                                 : "received")
                      << ") ";
            ++i;
...
void piece_download::deregister_peer(const peer_id_type& id)
{
    auto peer = std::find_if(
            peers_.begin(), peers_.end(), [&id](const auto& p) { return p.id == id; });
    if(peer == peers_.end()) {
        return;
    }
    // dissasociate peer from the blocks it had requested
    for(auto& block : peer->blocks) {
        block.get().remove_peer(id);
    }
    peers_.erase(peer);
}
...
inline bool piece_download::has_lingered_too_long(const block& block, const int m) const
        noexcept
{
    ...
    const milliseconds t = util::clamp(
            avg_request_rtt_ * m, milliseconds(min), milliseconds(seconds(20)));
