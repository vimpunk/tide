...
std::string bencode_error_category::message(int env) const
{
    ...
    case bencode_errc::bmap_keys_unordered:
        return "Keys in bmap not alphabetically ordered";
...
std::error_condition bencode_error_category::default_error_condition(int ev) const
        noexcept
{
...
public:
    ...
    std::unique_ptr<belement> decode(std::error_code& error)
    {
        ...
        if(c == 'd') {
            ...
            if(!error) {
                return std::make_unique<bmap>(std::move(map));
            }
        } else if(c == 'l') {
            blist list = decode_list(error);
            if(!error) {
                return std::make_unique<blist>(std::move(list));
            }
        } else if(c == 'i') {
...
string_view bcontainer::encode() const
{
    ...
    return string_view(source().c_str() + head()->offset,
            source().c_str() + last_element_offset + 1);
}

void format_map(std::stringstream& ss, const bcontainer& map, const btoken* head,
        int nesting_level)
{
    ss << '{';
    if(!map.head()) {
    ...
    const btoken* token = head ? head : map.head();
    ...
    while(token != map_end) {
        ...
        switch(token->type) {
        case btype::number: ss << make_number_from_token(map.source(), *token); break;
        case btype::string:
            ss << '"' << make_string_from_token(map.source(), *token) << '"';
            break;
        case btype::list: format_list(ss, map, token, nesting_level + 1); break;
        case btype::map: format_map(ss, map, token, nesting_level + 1); break;
        }
        token += token->next_item_array_offset;
        if(token != map_end) {
            ss << ',';
...
void format_list(std::stringstream& ss, const bcontainer& list, const btoken* head,
        int nesting_level)
{
    ss << '[';
    if(!list.head()) {
    ...
    const btoken* token = head ? head : list.head();
    ...
    while(token != list_end) {
        ...
        switch(token->type) {
        case btype::number: ss << make_number_from_token(list.source(), *token); break;
        case btype::string:
            ss << '"' << make_string_from_token(list.source(), *token) << '"';
            break;
        case btype::list: format_list(ss, list, token, nesting_level + 1); break;
        case btype::map: format_map(ss, list, token, nesting_level + 1); break;
        }
        token += token->next_item_array_offset;
        if(token != list_end) {
            ss << ", ";
...
const btoken* bmap::find_token(const std::string& key, const btoken* start_pos) const
        noexcept
{
    if(!head()) {
        return nullptr;
    ...
    //&& source().length() - token->offset - token->length >= key.length())
    {
        ...
        if(std::equal(key.c_str(), key.c_str() + key.length(), encoded_key_start,
...
const btoken* bmap::find_token_in_list(const std::string& key, const btoken* token) const
        noexcept
{
    const btoken* const list_end = token + token->next_item_array_offset;
